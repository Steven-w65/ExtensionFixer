...
class ReleaseAssetTestCase(unittest.TestCase):
    ...
    def test_required_release_assets_and_settings_schema(self):
        ...
        self.assertIn("name: extensionfixer-windows-x64\n", normalized_workflow)
        self.assertNotIn("name: extensionfixer-windows-x64-${{ github.sha }}", normalized_workflow)
        self.assertIn("path: dist/extensionfixer/", normalized_workflow)
        self.assertNotIn("compress-archive", normalized_workflow)
        self.assertNotIn("sha256", normalized_workflow)
        self.assertNotIn("checksum", normalized_workflow)
